...
def test_blank_dividends_treated_as_zero(fa, market, lot_row, cols):
    fa_row, _ = fa.process_row(lot_row(dividends=float("nan")), [])
    assert fa_row[cols["dividends"]] == 0.0


def test_zero_dividends_need_no_year_end_rate_when_sold(fa, market, lot_row, sale, cols):
    fa._RATE_CACHE["USD"] = fa._RATE_CACHE["USD"].iloc[:-1]
    fa_row, _ = fa.process_row(lot_row(dividends=0), [sale("2026-04-01", 10, 18)])
    assert fa_row[cols["dividends"]] == 0.0


def test_dividends_on_sold_lot_need_year_end_rate(fa, market, lot_row, sale):
    fa._RATE_CACHE["USD"] = fa._RATE_CACHE["USD"].iloc[:-1]
